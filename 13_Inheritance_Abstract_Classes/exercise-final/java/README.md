...
To allow polymorphism and reuse common code, all the other walls inherit from a `Wall` class. But it doesn't include any dimensions of its own, so you can't use it in the application as a wall.

If you open `PaintCalculator`, you'll see that some of the code is commented out. After you complete a step, you'll need to uncomment the code. After that, the code compiles.
